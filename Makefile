...
KREPO      = loudvents
KREPO_DESC = LoudVents Channel Provider
...
release: ## Publish container images and generate release manifests
	@mkdir -p $(DIST_DIR)
	$(KO) resolve -f config/ -l 'triggermesh.io/crd-install' > $(DIST_DIR)/loudvents-crds.yaml
	@cp config/namespace/100-namespace.yaml $(DIST_DIR)/loudvents.yaml
	$(KO) resolve $(KOFLAGS) -B -t latest -f config/ -l '!triggermesh.io/crd-install' > /dev/null
	$(KO) resolve $(KOFLAGS) -B -t $(IMAGE_TAG) --tag-only -f config/ -l '!triggermesh.io/crd-install' >> $(DIST_DIR)/loudvents.yaml
...
clean: ## Clean build artifacts
	...
	@$(RM) -v $(DIST_DIR)/loudvents-crds.yaml $(DIST_DIR)/loudvents.yaml
