...
func NewController(
	...
	loudventschannelInformer := loudventschannel.Get(ctx)
	...
	if err := envconfig.Process("", env); err != nil {
		logger.Panicf("unable to process loudvents channel's required environment variables: %v", err)
	}

	if env.Image == "" {
		logger.Panic("unable to process loudvents channel's required environment variables (missing DISPATCHER_IMAGE)")
	}

	r.dispatcherImage = env.Image

	impl := loudventschannelreconciler.NewImpl(ctx, r)
	r.uriResolver = resolver.NewURIResolverFromTracker(ctx, impl.Tracker)

	loudventschannelInformer.Informer().AddEventHandler(controller.HandleAll(impl.Enqueue))
