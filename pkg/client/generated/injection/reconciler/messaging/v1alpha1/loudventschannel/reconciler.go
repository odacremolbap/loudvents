...
// controller reconciling v1alpha1.LoudVentsChannel.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.LoudVentsChannel. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.LoudVentsChannel) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.LoudVentsChannel.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.LoudVentsChannel. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.LoudVentsChannel) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.LoudVentsChannel if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.LoudVentsChannel.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.LoudVentsChannel) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.LoudVentsChannel) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.LoudVentsChannel resources.
type reconcilerImpl struct {
	...
	Lister messagingv1alpha1.LoudVentsChannelLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client internalclientset.Interface, lister messagingv1alpha1.LoudVentsChannelLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.LoudVentsChannels(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1alpha1.LoudVentsChannel, desired *v1alpha1.LoudVentsChannel) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.MessagingV1alpha1().LoudVentsChannels(desired.Namespace)
		...
		updater := r.Client.MessagingV1alpha1().LoudVentsChannels(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.LoudVentsChannel) (*v1alpha1.LoudVentsChannel, error) {

	getter := r.Lister.LoudVentsChannels(resource.Namespace)
	...
	patcher := r.Client.MessagingV1alpha1().LoudVentsChannels(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.LoudVentsChannel) (*v1alpha1.LoudVentsChannel, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.LoudVentsChannel, reconcileEvent reconciler.Event) (*v1alpha1.LoudVentsChannel, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
